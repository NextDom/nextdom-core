...
step_6_nextdom_download() {
	...
	if [ $? -ne 0 ]; then
		...
		if [ -f /root/nextdom.zip ]; then
			cp /root/nextdom.zip /tmp/nextdom.zip
		fi
	fi
	if [ ! /tmp/nextdom.zip ]; then
		echo "${ROUGE}Ne peut trouver l'archive nextdom.zip - Annulation${NORMAL}"
    	exit 1
	fi
	mkdir -p ${WEBSERVER_HOME}
	find ${WEBSERVER_HOME} ! -name 'index.html' -type f -exec rm -rf {} +
	rm -rf /root/core-*
	unzip -q /tmp/nextdom.zip -d /root/
	if [ $? -ne 0 ]; then
