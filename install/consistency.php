...
try {
    ...
    if (config::byKey('object:summary') == '' || !is_array(config::byKey('object:summary'))) {
        config::save('object:summary',
            array('security' => array('key' => 'security', 'name' => 'Alerte', 'calcul' => 'sum', 'icon' => '<i class="icon nextdom-alerte2"></i>', 'unit' => '', 'count' => 'binary', 'allowDisplayZero' => false),
    ...
    $cron = cron::byClassAndFunction('nextdom', 'persist');
    if (is_object($cron)) {
        $cron->remove();
    }
    ...
    $cron = cron::byClassAndFunction('nextdom', 'updateSystem');
    if (is_object($cron)) {
        $cron->remove();
    }

    $cron = cron::byClassAndFunction('nextdom', 'checkAndCollect');
    if (is_object($cron)) {
        $cron->remove();
    }
