...
<body>
    <center>
        <img src="../core/img/logo-nextdom-grand-nom-couleur-460x320.png" class="img-responsive" />
    </center>
    <?php
$error = false;
                ...
                function loadLog(){
                    $( "#pre_installLog" ).load( "setup.php?log=1&key=<?php echo $tmp_key ?>", function(data) {
                        if(data.indexOf('[END INSTALL SUCCESS]') > 0){
                            $('#div_alertMessage').removeClass('alert-warning').addClass('alert-success').html('<center style="font-size:1.2em;"><i class="fa fa-check"></i> NextDom successfully install. Login is <strong>admin</strong>, password is <strong>admin</strong>. Click <a href="../index.php">here</a> for connection</center>');
                            return;
                        }
