...
if (init('type') != '') {
    ...
    try {
        ...
        if ((!nextdom::apiAccess(init('apikey', init('api')), init('plugin', 'core')) &&
            !nextdom::apiAccess(init('apikey', init('api')), init('type', 'core'))) ||
            (!nextdom::apiModeResult(config::byKey('api::core::http::mode', init('plugin', 'core'), 'enable')) &&
                !nextdom::apiModeResult(config::byKey('api::core::http::mode', init('type', 'core'), 'enable')))) {
            user::failedLogin();
            sleep(5);
...
try {
    ...
    if (isset($params['plugin']) && $params['plugin'] != '' && $params['plugin'] != 'core') {
        ...
        if ($jsonrpc->getMethod() == 'cmd::save') {
            ...
            utils::a2o($cmd, nextdom::fromHumanReadable($params));
            $cmd->save();
