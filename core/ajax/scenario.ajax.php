...
try {
    ...
    if (init('action') == 'testExpression') {
        $return = array();
        $scenario = null;
        $return['evaluate'] = scenarioExpression::setTags(nextdom::fromHumanReadable(init('expression')), $scenario, true);
    ...
    if (init('action') == 'emptyLog') {
        ...
        if (file_exists(__DIR__ . '/../../var/logs/scenarioLog/scenario' . $scenario->getId() . '.log')) {
            unlink(__DIR__ . '/../../var/logs/scenarioLog/scenario' . $scenario->getId() . '.log');
        }
        ajax::success();
    }
