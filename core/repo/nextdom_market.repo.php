...
class repo_nextdom_market {
    ...
    public static function checkUpdate(&$_update) {
        if (is_array($_update)) {
            ...
            $markets_infos = repo_nextdom_market::getInfo($markets['logicalId'], $markets['version']);
        ...
        $market_info = repo_nextdom_market::getInfo(array('logicalId' => $_update->getLogicalId(), 'type' => $_update->getType()), $_update->getConfiguration('version', 'stable'));
    ...
    public static function downloadObject($_update) {
        $market = repo_nextdom_market::byLogicalIdAndType($_update->getLogicalId(), $_update->getType());
        if (is_object($market)) {
    ...
    public static function deleteObjet($_update) {
        try {
            ...
            $market = new repo_nextdom_market();
    ...
    public static function objectInfo($_update) {
        ...
        if ($_update->getConfiguration('third_plugin', null) === null) {
            ...
            if (strpos($header[0], '200') === false) {
                ...
                $url = 'https://nextdom.github.io/documentation/third_plugin/' . $_update->getLogicalId() . '/' . config::byKey('language', 'core', 'fr_FR') . '/index.html';
            }
            $_update->save();
    ...
    public static function sendBackupCloud($_path, $_chunksize = 4096000) {
        ...
        while (!$backup["completed_at"]) {
            try {
                $backup = repo_nextdom_market::uploadChunk($_path, $backup["chunk_start"], $backup["chunk_end"], $backup["id"], $fileSize);
            } catch (Exception $e) {
                echo $e->getMessage() . "\n";
                repo_nextdom_market::sendBackupCloud($_path);
                break;
            }
    ...
    public static function retoreBackup($_backup) {
        $url = config::byKey('market::address') . "/core/php/downloadBackup.php?backup=" . $_backup . '&hwkey=' . nextdom::getHardwareKey() . '&username=' . urlencode(config::byKey('market::username')) . '&password=' . self::getPassword() . '&password_type=sha1';
        $tmp_dir = nextdom::getTmpFolder('market');
        $tmp = $tmp_dir . '/' . $_backup;
        $opts = array(
            "ssl" => array(
                "verify_peer" => false,
        ...
        nextdom::restore('backup/' . $_backup, true);
    }
    ...
    public static function getInfo($_logicalId, $_version = 'stable') {
        ...
        if (is_array($_logicalId) && is_array($_version) && count($_logicalId) == count($_version)) {
            ...
            for ($i = 0; $i < $countLogicalId; $i++) {
                ...
                try {
                    ...
                    log::add('market', 'debug', __('Erreur repo_nextdom_market::getinfo : ', __FILE__) . $e->getMessage());
                    $return['status'] = 'ok';
                } catch (Error $e) {
                    log::add('market', 'debug', __('Erreur repo_nextdom_market::getinfo : ', __FILE__) . $e->getMessage());
                    $return['status'] = 'ok';
        ...
        try {
            if (is_array($_logicalId)) {
                $market = repo_nextdom_market::byLogicalIdAndType($_logicalId['logicalId'], $_logicalId['type']);
            } else {
                $market = repo_nextdom_market::byLogicalId($_logicalId);
            }
            if (!is_object($market)) {
    ...
    public static function saveTicket($_ticket) {
        ...
        $_ticket['options']['nextdom_version'] = nextdom::version();
    ...
    public static function getItemsList() {
        $result = [];
        $item = new NextDomMarketItem();
        $item->setCategory('programmation');
        $item->setLogicialId('AlternativeMarketForJeedom');
        $result[] = $item;
        return $result;
    }
    public static function getJsonRpc() {
        return self::getItemsList();
        /*
        $internalIp = '';
        try {
        ...
        $jsonrpc->setCb_class('repo_nextdom_market');
    ...
    public static function postJsonRpc(&$_result) {
        ...
        if (is_array($_result)) {
            ...
            if (config::byKey('market::allowDNS') == 1) {
                if (isset($_result['nextdom::url']) && config::byKey('nextdom::url') != $_result['nextdom::url']) {
                    config::save('nextdom::url', $_result['nextdom::url']);
                }
            }
    ...
    public function install($_version = 'stable') {
        ...
        $url = config::byKey('market::address') . "/core/php/downloadFile.php?id=" . $this->getId() . '&version=' . $_version . '&nextdomversion=' . nextdom::version() . '&hwkey=' . nextdom::getHardwareKey() . '&username=' . urlencode(config::byKey('market::username')) . '&password=' . self::getPassword() . '&password_type=sha1';
    ...
    public function save() {
        ...
        switch ($this->getType()) {
            case 'plugin':
                ...
                $cibDir = nextdom::getTmpFolder('market') . '/' . $plugin_id;
                ...
                $tmp = nextdom::getTmpFolder('market') . '/' . $plugin_id . '.zip';
                if (file_exists($tmp)) {
                    if (!unlink($tmp)) {
