...
if (init('id') != '') {
    $market = repo_nextdom_market::byId(init('id'));
}
if (init('logicalId') != '' && init('type') != '') {
    $market = repo_nextdom_market::byLogicalIdAndType(init('logicalId'), init('type'));
}
if (!isset($market)) {
...
if ($market->getPurchase() == 1) {
    ...
    if (config::byKey('market::apikey') != '' || (config::byKey('market::username') != '' && config::byKey('market::password') != '')) {
        $purchase_info = repo_nextdom_market::getPurchaseInfo();
