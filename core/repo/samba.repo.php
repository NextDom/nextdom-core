...
class repo_samba {
    ...
    public static function downloadObject($_update) {
        $tmp_dir = nextdom::getTmpFolder('samba');
    ...
    public static function downloadCore($_path) {
        $pathinfo = pathinfo($_path);
        $cmd = 'cd ' . $pathinfo['dirname'] . ';';
        $cmd .= self::makeSambaCommand('get ' . config::byKey('samba::core::path') . '/nextdom.zip', 'plugin');
        com_shell::execute($cmd);
    ...
    public static function versionCore() {
        try {
            if (file_exists(nextdom::getTmpFolder('samba') . '/version')) {
                com_shell::execute(system::getCmdSudo() . 'rm /tmp/nextdom_version');
            }
            $cmd = 'cd /tmp;';
            $cmd .= self::makeSambaCommand('get ' . config::byKey('samba::core::path') . '/nextdom_version', 'plugin');
            com_shell::execute($cmd);
            if (!file_exists(nextdom::getTmpFolder('samba') . '/version')) {
                return null;
            }
            $version = trim(file_get_contents(nextdom::getTmpFolder('samba') . '/version'));
            com_shell::execute(system::getCmdSudo() . 'rm ' . nextdom::getTmpFolder('samba') . '/version');
            return $version;
        } catch (Exception $e) {
