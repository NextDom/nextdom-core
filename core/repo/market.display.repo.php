                ...
                if (config::byKey('market::apikey') != '' || (config::byKey('market::username') != '' && config::byKey('market::password') != '')) {
                    ...
                    if (isset($purchase_info['user_id']) && is_numeric($purchase_info['user_id'])) {
                        ...
                        if ($market->getCertification() === 'Premium') {
                            ...
                            echo '<a class="btn btn-default" target="_blank" href="' . config::byKey('market::address') . '/index.php?v=d&p=purchaseItem&user_id=' . $purchase_info['user_id'] . '&type=plugin&id=' . $market->getId() . '"><i class="fas fa-shopping-cart spacing-right"></i>{{Acheter}}</a>';
                        }
                    } else {
