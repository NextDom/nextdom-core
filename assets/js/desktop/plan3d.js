...
function add3dObject(_info){
    ...
    if(nextdom3d[_info.configuration['3d::widget']]){
        if(nextdom3d[_info.configuration['3d::widget']].reset){
            nextdom3d[_info.configuration['3d::widget']].reset(_info,object);
        }
        nextdom3d[_info.configuration['3d::widget']].create(_info,object);
    }
}
...
nextdom3d.light = function() {};

nextdom3d.light.create = function(_info,_object) {
    ...
    NEXTDOM_OBJECT.push(light.uuid);
    scene.add(light);
    ...
    nextdom3d.light.update({display_value : _info.additionalData.state, cmd_id : _info.additionalData.cmd_id});
};

nextdom3d.light.update = function(_options) {
...
nextdom3d.text = function() {};
nextdom3d.text.data = {};

nextdom3d.text.reset = function(_info,_object){
    if(!nextdom3d.text.data[_object.uuid]){
        return;
    }
...
nextdom3d.text.create = function(_info,_object) {
    var text = nextdom3d.text.generate(_info,_object,_info.additionalData.text);
    scene.add(text);
    nextdom3d.text.data[_object.uuid] = text.uuid;
...
nextdom3d.text.generate = function(_options,_object,_text){
    ...
    var spritey = nextdom3d.text.makeTextSprite(_text,
        {
...
nextdom3d.text.makeTextSprite = function( message, parameters ){
    message = " " + message + " ";
    ...
    var textWidth = nextdom3d.text.getMaxWidth(context, texts);
    ...
    nextdom3d.text.roundRect(context, (size/2 - textWidth / 2) - borderThickness/2, size / 2 - fontsize/2 - totalTextHeight/2, textWidth + borderThickness, totalTextHeight + fontsize/2 , 6);
...
nextdom3d.text.roundRect = function(ctx, x, y, w, h, r) {
    ctx.beginPath();
    ctx.moveTo(x+r, y);
    ctx.lineTo(x+w-r, y);
...
nextdom3d.text.getMaxWidth =  function(context, texts){
    let maxWidth = 0;
    for(let i in texts)
...
nextdom3d.door = function() {};
nextdom3d.door.data = {};

nextdom3d.door.reset = function(_info,_object){
    if(nextdom3d.door.data[_object.uuid]){
        _object.material.color = nextdom3d.door.data[_object.uuid];
    }
}
...
nextdom3d.door.create = function(_info,_object) {
    ...
    for(var i in _info.additionalData.cmds){
        ...
        nextdom3d.door.update({state : _info.additionalData.state, cmd_id : cmd_id,object : _object});
    }
};
...
nextdom3d.door.update = function(_options) {
    ...
    for(var i in doors){
        ...
        if(_options.state){
            CMDS[_options.cmd_id]['door'][i].info.lastPosition=nextdom3d.door.doUpdate(_options.state,doors[i]);
        }else{
            nextdom.plan3d.byId({
                id: doors[i].info.id,
                global:false,
                async : false,
                success: function (data) {
                    CMDS[_options.cmd_id]['door'][i].info.lastPosition=nextdom3d.door.doUpdate(data.additionalData.state,doors[i]);
                }
            });
        }
    }
...
nextdom3d.door.doUpdate = function(_state,_door){
    var result = ''
    var convert = {
...
nextdom3d.conditionalColor = function() {};
nextdom3d.conditionalColor.data = {};

nextdom3d.conditionalColor.reset = function(_info,_object){
    if(nextdom3d.conditionalColor.data[_object.uuid]){
        _object.material.color = nextdom3d.conditionalColor.data[_object.uuid];
    }
}

nextdom3d.conditionalColor.create = function(_info,_object) {
    _object.material = _object.material.clone();
    for(var i in _info.additionalData.cmds){
        cmd_id = _info.additionalData.cmds[i];
        if(! CMDS[cmd_id]){
    ...
    nextdom3d.conditionalColor.update({color : _info.additionalData.color, cmd_id : cmd_id,object : _object});
};
    ...
    for(var i in conditionalColor){
        ...
        if(_options.color){
            if(!nextdom3d.conditionalColor.data[conditionalColor[i].object.uuid]){
                nextdom3d.conditionalColor.data[conditionalColor[i].object.uuid] = {r:conditionalColor[i].object.material.color.r,g:conditionalColor[i].object.material.color.g,b:conditionalColor[i].object.material.color.b};
            }
...
nextdom3d.conditionalShow = function() {};

nextdom3d.conditionalShow.reset = function(_info,_object){
    _object.visible = true;
}

nextdom3d.conditionalShow.create = function(_info,_object) {
    ...
    nextdom3d.conditionalShow.update({show : _info.additionalData.show, cmd_id : cmd_id,object : _object});
};
