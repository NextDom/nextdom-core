...
<script>
  ...
  $('body').off('click','.bt_selectOtherActionExpression').on('click','.bt_selectOtherActionExpression',  function (event) {
    ...
    nextdom.getSelectActionModal({scenario : true}, function (result) {
     ...
     nextdom.cmd.displayActionOption(expression.find('.expressionAttr[data-l1key=cmd]').value(), '', function (html) {
  ...
  $('#bt_planConfigureSelectCamera').on('click', function() {
    var el = $(this);
    nextdom.eqLogic.getSelectModal({eqLogic: {eqType_name: 'camera'}}, function(result) {
