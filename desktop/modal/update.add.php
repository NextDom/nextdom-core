...
?>
 ...
 <div class="alert alert-danger">{{Attention, il n’y a pas d’assistance de l’équipe NextDom sur les plugins installés depuis une autre source que le Market NextDom. De plus, l’installation d’un plugin depuis une autre source que le Market nextdom peut dans certains cas entrainer la perte globale d’assistance par l’équipe NextDom.}}</div>
	...
	foreach ($value['configuration']['parameters_for_add'] as $pKey => $parameter) {
		...
		switch ($parameter['type']) {
			...
			case 'file':
				...
				echo '<i class="fa fa-cloud-upload"></i> {{Envoyer un plugin}}<input id="bt_uploadPlugin" data-key="' . $pKey . '" type="file" name="file" data-url="core/ajax/update.ajax.php?action=preUploadFile&nextdom_token=' . ajax::getToken() . '" style="display : inline-block;">';
				echo '</span>';
				break;
		}
		echo '</div>';
