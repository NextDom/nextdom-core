...
<head>
    ...
    if (!isConnect()) {
        ...
        try {
            ...
            if (init('rescue', 0) == 0 &&
                file_exists(NEXTDOM_ROOT . 'core/themes/' . $bootstrapTheme . '/desktop/' . $bootstrapTheme . '.css')) {
                include_file('core', $bootstrapTheme . '/desktop/' . $bootstrapTheme, 'themes.css');
            } else if (
                init('rescue', 0) == 0 && file_exists(NEXTDOM_ROOT . 'core/themes/' . $defaultBootstrapTheme . '/desktop/' . $defaultBootstrapTheme . '.css')) {
    ...
    include_file('core', 'icon.inc', 'php'); // Icons et fonts
    ...
    try {
        if (isConnect()) {
            if (init('rescue', 0) == 0 && is_dir(NEXTDOM_ROOT . '/core/themes/' . $bootstrapTheme . '/desktop')) {
                if (file_exists(NEXTDOM_ROOT . '/core/themes/' . $bootstrapTheme . '/desktop/' . $bootstrapTheme . '.js')) {
            ...
            if (init('rescue', 0) == 0 && $_SESSION['user']->getOptions('desktop_highcharts_theme') != '') {
                try {
                    if (is_dir(NEXTDOM_ROOT . '/core/themes/' . $bootstrapTheme . '/desktop')) {
                        if (file_exists(NEXTDOM_ROOT . '/core/themes/' . $bootstrapTheme . '/desktop/' . $bootstrapTheme . '.js')) {
                            include_file('core', $bootstrapTheme . '/desktop/' . $bootstrapTheme, 'themes.js');
                        }
                    }
