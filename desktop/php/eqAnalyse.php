...
foreach (eqLogic::all() as $eqLogic) {
    foreach ($eqLogic->getCmd('info') as $cmd) {
        if (count($cmd->getConfiguration('actionCheckCmd', array())) > 0) {
            ...
            echo '<td>Si ' . $cmd->getConfiguration('nextdomCheckCmdOperator') . ' ' . $cmd->getConfiguration('nextdomCheckCmdTest') . ' {{plus de}} ' . $cmd->getConfiguration('nextdomCheckCmdTime') . ' {{minutes alors}} : ';
            $actions = '';
        ...
        foreach ($listCmds as $cmdalert) {
            foreach ($NEXTDOM_INTERNAL_CONFIG['alerts'] as $level => $value) {
                ...
                if ($cmdalert->getAlert($level . 'if', '') != '') {
                    ...
                    echo ucfirst($level) . ' si ' . nextdom::toHumanReadable(str_replace('#value#', '<b>' . $cmdalert->getName() . '</b>', $cmdalert->getAlert($level . 'if', ''))) . $during . '</br>';
                }
            }
        }
        echo '</td>';
...
foreach (nextdom::deadCmd() as $datas) {
    echo '<tr>';
    echo '<td>Core</td>';
