...
<script type="text/javascript">
	...
	function page_rebootjs(rebooti){
		refresh();
		if(rebooti=='1'){
			$('#div_reboot_nextdom_texte').empty().html('<h6>NextDom est de nouveau opérationnel. Vous allez être redirigé vers votre dashboard.</h6>');
			...
			if(testnextdom > '15'){
				...
				$('#div_reboot_nextdom_texte').empty().html('<h6>NextDom n\'a pas encore redémarré, nous continuons cependant de tester son retour.<br />N\'hésitez pas à la débrancher/rebrancher électriquement.</h6>');
			}
		}
	}

	function reboot_nextdom(rebooti){
		$('#div_reboot_nextdom_texte').empty().html('<h6>Merci de patienter...<br />NextDom est en cours de redémarrage.</h6>');
