...
-   Ajout de l'action nextdom_reboot pour lancer un redémarrage de NextDom.
...
-   Ajout d'une commande dans les scénarios : value(commande) permet d'avoir la valeur d'une commande si elle n'est pas donnée automatiquement par nextdom (cas lors du stockage du nom de la commande dans une variable).
...
-   Ajout d’un watchdog indépendant de NextDom pour vérifier le status de
    MySql et Apache
