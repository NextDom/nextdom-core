...
-   ** ** El acceso interno: información para la unión de un NextDom
    ...
    -   ** ** Puerto: el puerto de la NextDom interfaz web, por lo general 80.
...
> \\\\ 192.168.0.1 \\ Copia de seguridad \\ Domótica \\ NextDom entonces IP = 192.168.0.1
>, Recurso compartido = //192.168.0.1/Copia de seguridad, Carpeta = Domótica / NextDom

> **Nota**
>
> Al validar el recurso compartido de Samba descrito,
> aparece una nueva forma de copia de seguridad en la sección
> Administración → Copias de seguridad de NextDom. Al activarlo, NextDom procederá
...
> esté en el NAS. A continuación, debe editar en el sistema operativo de su NextDom
> el archivo /etc/samba/smb.conf y agregar estas dos líneas:
> client max protocol = SMB3
> cliente min protocol = SMB2
> El smbclient de NextDom usará v2 o v3 y, poniendo SMB3 a los dos(max y min), usará solamente
> SMB3. Depende de usted adaptarse según las restricciones del lado del NAS u otro servidor de Samba

> **Importante**
>
> NextDom debe ser el único que escriba en esta carpeta y debe estar vacía
