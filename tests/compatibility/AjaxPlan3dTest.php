...
class AjaxPlan3dTest extends AjaxBase
{
    ...
    public function testGetAsUser() {
        ...
        $this->assertStringContainsString('Aucun plan3d correspondant', (string) $result->getBody());
        $this->assertEquals(200, $result->getStatusCode());
    }
    ...
    public function testGetPlan3dHeaderAsUser() {
        ...
        $this->assertStringContainsString('plan3d header inconnu verifiez', (string) $result->getBody());
        $this->assertEquals(200, $result->getStatusCode());
    }
    ...
    public function testSavePlan3dHeaderAsAdmin() {
        $this->connectAsAdmin();
        $result = $this->getAjaxQueryWithTokenResult($this->ajaxFile, ['action' => 'saveplan3dHeader']);
        $this->assertStringContainsString('Le nom du l\'objet ne peut pas être vide', (string) $result->getBody());
        $this->assertEquals(200, $result->getStatusCode());
    }
