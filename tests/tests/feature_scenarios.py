...
class ScenariosTest(unittest.TestCase):
    ...
    def test_scenario_cron_execution(self):
        ...
        shell_output = subprocess.check_output(
            'docker exec -it nextdom-test-scenarios ls /var/log/nextdom/scenarioLog'.split(' '))  # pylint: disable=line-too-long
        self.assertIn('scenario1.log', shell_output.decode('utf-8'))
        shell_output = subprocess.check_output(
            'docker exec -it nextdom-test-scenarios cat /var/log/nextdom/scenarioLog/scenario1.log'.split(' '))  # pylint: disable=line-too-long
        self.assertIn('automatiquement sur programmation',
                      shell_output.decode('utf-8'))
