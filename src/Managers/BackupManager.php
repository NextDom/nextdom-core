...
class BackupManager
{
    ...
    public static function restoreBackup($file = '') {
        ...
        try {
            ConsoleHelper::title("starting restore procedure at ".date('Y-m-d H:i:s')."\n");
            NextDomHelper::event('begin_restore', true);

            if ($file === null) {
                $file = self::getLastBackupFilePath($backupDir, "newest");
            }
            ConsoleHelper::step("file used for restoration: %s\n", $file);
            ...
            ConsoleHelper::ok();
            ConsoleHelper::step("processing Migration/Upgrade");
            MigrationHelper::migrate($tmpDir);
            ConsoleHelper::ok();
