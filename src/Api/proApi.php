...
try {
    ...
    if (!nextdom::apiModeResult(config::byKey('api::core::pro::mode', 'core', 'enable'))) {
    ...
    if (isset($params['proapi']) && !nextdom::apiAccess($params['proapi'], 'apipro')) {
        throw new Exception(__('Vous n\'êtes pas autorisé à effectuer cette action', __FILE__), -32001);
    }
    ...
    if (isset($params['plugin']) && $params['plugin'] != '') {
        ...
        if ($jsonrpc->getMethod() == 'eqLogic::save') {
            ...
            if (isset($params['cmd'])) {
                ...
                foreach ($params['cmd'] as $cmd_info) {
                    ...
                    utils::a2o($cmd, nextdom::fromHumanReadable($cmd_info));
                    $cmd->save();
                    $cmd_order++;
        ...
        if ($jsonrpc->getMethod() == 'jeeNetwork::handshake') {
            if (config::byKey('jeeNetwork::mode') != 'slave') {
                throw new Exception(__('Impossible d\'ajouter une box NextDom non esclave à un réseau NextDom', __FILE__));
            }
        ...
        if ($jsonrpc->getMethod() == 'backup::list') {
            $jsonrpc->makeSuccess(nextdom::listBackup());
        }
        ...
        if ($jsonrpc->getMethod() == 'backup::remove') {
            nextdom::removeBackup($params['backup']);
            $jsonrpc->makeSuccess();
        }

        if ($jsonrpc->getMethod() == 'backup::restore') {
            nextdom::restore($params['backup'], true);
            $jsonrpc->makeSuccess();
        }
